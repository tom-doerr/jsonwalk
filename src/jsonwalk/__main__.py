"""Command dispatch.
    ...
    jsonwalk edit FILE -o "..."       propose edits to a text artifact
    jsonwalk                          interactive TUI
    jsonwalk tui                      TUI, explicit

Bare arguments go to the CLI so the tool behaves like a normal command;
``run``, ``tui`` and ``edit`` are reserved as the first word, which is why a
field literally named "run" needs the explicit ``jsonwalk run run ...`` form.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and argv[0] == "edit":
        from .edit_cli import main as edit_main
